...
@Component({
  selector: 'app-unite-structurelle-create-admin',
  templateUrl: './unite-structurelle-create-admin.component.html'
})
export class UniteStructurelleCreateAdminComponent  implements OnInit {
   ...
   private _validUniteStructurelleCode = true;
    ...
    const isPermistted = await this.roleService.isPermitted('UniteStructurelle', 'add');
    if(isPermistted) {
    ...
    const isPermistted = await this.roleService.isPermitted('Genre', 'add');
    if(isPermistted) {
